...
public class Gun : MonoBehaviour
{
	...
	// 장전 소리 재생기
	public  AudioClip reloadOutAudio;
	public AudioClip reloadInAudio;
	...
	{
		// 총의 Reload 트리거 발동 (애니메이터에서 알아서 함)
		if(state == GUN_STATE.EMPTY){
			gunAnimator.SetBool("Empty", true);
		...
		audioSource.clip = reloadOutAudio;
		audioSource.PlayScheduled(0.5f);

		audioSource.clip = reloadInAudio;
